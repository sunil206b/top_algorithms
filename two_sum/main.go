...
// of numbers but that would be too slow. Again, it's best to try out
...
func main() {
	nums := []int{3, 2, 4}
	target := 6
	fmt.Println(twoSumBasic(nums, target))
	nums1 := []int{2, 7, 11, 15}
	target1 := 9
	fmt.Println(twoSumBasic(nums1, target1))
}
...
func twoSumBasic(nums []int, target int) []int {
	...
	for i, num := range nums {
		diff := target - num
		if index, ok := numIndices[diff]; ok {
			return []int{index, i}
		}
